...
const usePostList = () => {
    ...
    const addPost = async (title: string, description: string, content: string, userName: string, userId: string, avatar: string) => {
        ...
        try {
            ...
            if (response.status === 201) {
                ...
                await swal(intl.formatMessage({id: "Add_Post_Successfully"}), "", "success");
            }
        } catch (error) {
            await swal(intl.formatMessage({id: "Add_Post_Failed"}), "", "warning");
        }
    ...
    const deletePost = async (_id: string) => {
        ...
        try {
            const response = await axios.delete(`/api/posts/${_id}`, {data: deleteInfo});
            if (response.status === 204) {
                setPostList(await getPostList());
                await swal(intl.formatMessage({id: "Delete_Post_Successfully"}), "", "success");
            }
            return response.status;
        } catch (error) {
            await swal(intl.formatMessage({id: "Delete_Post_Failed"}), "", "warning");
